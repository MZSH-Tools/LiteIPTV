...
| 功能 | 说明 | 状态 |
|------|------|------|
| 抓取 | 从多个上游源获取 IPTV 直播地址 | [x] |
| 筛选 | 只保留 CCTV 频道（1-17 + 5+），分离 IPv4/IPv6 地址 | [x] |
| 测速 | 多轮测速，综合评估选择最稳定的源 | [x] |
| 输出 | 生成 `ipv4.m3u` 和 `ipv6.m3u` 两个文件 | [x] |
| 自动更新 | 定时运行，保持源的有效性 | [x] |
